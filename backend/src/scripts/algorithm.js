...
export class Class {
  constructor(crn, numGroup = 0) {
    ...
    // console.log(JSON.stringify(result), crn);
    ...
    for (let i = 0; i < result.length; i++) {
      for (let j = 0; j < result[i].days.length; j++) {
        ...
        this.endTime[id] = parseInt(
          bruh[1].substring(0, 2) * 60 + parseInt(bruh[1].substring(2, 4)),
        );
...
export class Schedule {
  constructor(classes) {
    ...
    for (let i = 0; i < this.numClasses; i++) {
      console.log(classes[i].professor);
...
function makeSchedule(i) {
  ...
  // console.log(JSON.stringify(ClassGroups, "this", ClassGroups[i].length));
